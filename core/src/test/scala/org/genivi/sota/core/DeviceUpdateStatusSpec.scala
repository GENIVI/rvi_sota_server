...
import org.genivi.sota.data.{DeviceSearchCommon, UpdateStatus}
...
class DeviceUpdateStatusSpec extends FunSuite
  ...
  test("Error if the last package is in Failed State") {
    ...
    val result = DeviceSearchCommon.currentDeviceStatus(device.lastSeen, packages)
    result shouldBe Error
  }

  test("up to date if there are no specs and vehicle was seen") {
    val result = DeviceSearchCommon.currentDeviceStatus(device.lastSeen, List.empty)
  ...
  test("not seen if device was never seen") {
    val result = DeviceSearchCommon.currentDeviceStatus(None, List.empty)
