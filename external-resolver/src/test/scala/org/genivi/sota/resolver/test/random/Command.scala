...
final case class InstallPackage  (veh: Uuid, pkg: Package) extends WellFormedCommand
final case class UninstallPackage(veh: Uuid, pkg: Package) extends WellFormedCommand
...
final case class InstallPackageFail  (veh: Uuid, pkg: Package) extends InvalidCommand
final case class UninstallPackageFail(veh: Uuid, pkg: Package) extends InvalidCommand
...
trait InvalidCommandUtils { _: CommandUtils =>
  ...
  def getBadReqInstallPackage(veh: Uuid, pkg: Package)
                             (implicit ec: ExecutionContext): Semantics =
    Semantics(installPackage(veh, pkg), StatusCodes.BadRequest, Success)

  def getBadReqUninstallPackage(veh: Uuid, pkg: Package)
                               (implicit ec: ExecutionContext): Semantics =
  ...
  def xlateToInvalidCommand(cmd: WellFormedCommand): InvalidCommand = cmd match {
    ...
    case InstallPackage(veh0, pkg0) =>
      val (veh: Uuid, pkg: Package) = pickOneOf(
