...
The GENIVI SOTA project includes a link:https://github.com/advancedtelematic/rvi_sota_client[client implementation] in link:http://rust-lang.org[Rust] for the GENIVI platform, but any client that implements the necessary JSON-RPC calls may be used. This document is a reference doc for those calls. You can also take a look at a detailed breakdown of a complete session, from registration through to completed package installation, link:/rvi_sota_server/dev/client-implementation-example-session.html[here].
