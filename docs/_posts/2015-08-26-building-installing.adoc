...
All client-server communications are done using the link:https://github.com/PDXostc/rvi_core[RVI protocol]. To get an RVI node suitable for SOTA up and running, you should

?? use our docker images ??

Get around using an RVI node by modifying core/Boot.scala line 57 like so:

[source,scala]
----
sotaServices <- FastFuture.successful(ServerServices("","","","")) // SotaServices.register( Uri(config.getString("rvi.sotaServicesUri")) )
----
